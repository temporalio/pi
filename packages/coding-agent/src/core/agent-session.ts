...
export class AgentSession {
	...
	/** The retry budget as the transcript shows it, for a caller that carries no count of its own. */
	...
	/** What every run ends with, whether it ran a whole turn or one step of it. */
	...
	async sealStep(
		...
		try {
			return await this._sealStep(toolCalls, options);
		} finally {
			await this._settleRun();
		}
	}
	...
	private async _sealStep(
		...
		return { done: !outcome.hasMoreToolCalls && !needsAnotherPass, retryAttempt: this._retryAttempt };
	}
	...
	private async _postSealPass(retryAttempt?: number): Promise<boolean> {
		...
		if (!this._lastAssistantMessage) {
			...
			// is taken, and failing that the transcript's trailing errors, which a compaction can have
			// rewritten: a caller that keeps the count somewhere durable is believed over them.
